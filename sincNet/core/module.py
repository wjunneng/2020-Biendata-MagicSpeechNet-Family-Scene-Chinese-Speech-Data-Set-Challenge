...
from sincNet.lib.util import Util
...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=16000, in_channels=1, stride=1, padding=0, dilation=1,
        ...
        if in_channels != 1:
            # msg = (f'SincConv only support one input channel f'(here, in_channels = {in_channels:d}).')
...
class sinc_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.N_filt):
            low_pass1 = 2 * filt_beg_freq[i].float() * Util.sinc(filt_beg_freq[i].float() * self.freq_scale, t_right)
            low_pass2 = 2 * filt_end_freq[i].float() * Util.sinc(filt_end_freq[i].float() * self.freq_scale, t_right)
...
class MLP(nn.Module):
    def __init__(self, options):
        ...
        for i in range(self.N_fc_lay):
            ...
            # activation
            self.act.append(Util.act_fun(self.fc_act[i]))
...
class SincNet(nn.Module):
    ...
    def __init__(self, options):
        ...
        for i in range(self.N_cnn_lay):
            ...
            # activation
            self.act.append(Util.act_fun(self.cnn_act[i]))
